...
/// Source Authority Repair (Phase C — closing the Family A Full Body
/// repair): proves `HypertrophyProgramGenerator`'s 6-Day Full Body
/// Mesocycle 1/2/3 now reflect the REAL, cell-cited structure recovered
/// from `source_workbooks/6 day full body.xlsx`, replacing the legacy
/// `generateLegacyFixedPair` placeholder (identical primary+paired slot
/// repeated every day) for this, the last remaining unrecovered Family A
/// Full Body configuration.
@MainActor
final class SixDayFullBodySourceFidelityTests: XCTestCase {
    ...
    private func generate(phaseType: HypertrophyPhaseType = .basicHypertrophy) throws -> ProgramDefinition {
        ...
        return try HypertrophyProgramGenerator.generate(
            configuration: HypertrophyProgramConfiguration(dayCount: 6, split: .fullBody, phaseType: phaseType),
    ...
    func testExactlySixDaysWithRealEmphasisNames() throws {
        let definition = try generate()
        XCTAssertEqual(definition.orderedTemplateSessions.map(\.name), [
            "Chest Focused Upper", "Quad Focused Lower", "Arms Focused Upper",
            "Glute Focused Lower", "Back Focused Upper", "Ham Calf Shoulder Focused",
        ])
    }
    ...
    func testDayOneMatchesTheRealRecoveredSourceCategorySequence() throws {
        ...
        XCTAssertEqual(slots.compactMap { $0.exerciseSlot?.name }, [
            "Incline Push", "Chest Isolation", "Horizontal Push", "Rear or Side Delts", "Horizontal Pull",
        ])
        ...
        XCTAssertEqual(baselineSets, [3, 3, 3, 2, 2])
    }

    /// The exact bug this repair fixes: 6 distinct, non-repeated daily
    /// slot sequences (30 slots total — "perfectly uniform 5 slots/day,"
    /// matching `SOURCE_PROGRAM_MANIFEST.md`'s own claim for this file)
    /// — never the legacy placeholder's single primary+paired pair
    /// repeated on every day.
    func testAllSixDaysAreDistinctNotTheLegacyRepeatedTwoSlotPlaceholder() throws {
        let definition = try generate()
        ...
        XCTAssertEqual(daySlotNames.map(\.count), [5, 5, 5, 5, 5, 5], "30 real slots/week, perfectly uniform 5/day")
        let uniqueDaySequences = Set(daySlotNames.map { $0.joined(separator: "|") })
        XCTAssertEqual(uniqueDaySequences.count, 6, "all 6 days must have genuinely distinct slot sequences, never the legacy repeated pair")
    }

    /// This workbook's own real category labels are singular ("Quad,"
    /// "Glute," "Hamstring Isolation," "Hamstring Hip Hinge") — confirmed
    /// via real cells, resolved through `SourceHypertrophyCategory
    /// .labelAliases`' new entries to the SAME categories every other
    /// Family A file labels in plural.
    func testWorkbookSingularLabelsResolveToTheSameCanonicalCategoriesAsOtherDayCounts() throws {
        let definition = try generate()
        let day2 = definition.orderedTemplateSessions[1]
        let quadSlot = try XCTUnwrap(day2.orderedBlockTemplates.first?.orderedPrescriptionTemplates.first?.exerciseSlot)
        XCTAssertEqual(quadSlot.name, "Quad", "the real workbook's own literal singular label is preserved for display/traceability")
        XCTAssertTrue(quadSlot.allowedTargets.contains(.quadriceps), "but the underlying category is the same real .quads semantic identity 3/4/5-Day already use")
    }
        ...
        for day in definition.orderedTemplateSessions {
            for template in day.orderedBlockTemplates.first?.orderedPrescriptionTemplates ?? [] {
                guard case .rmBased(let payload) = template.rules?.loadRule else {
                    return XCTFail("every 6-Day Mesocycle 1 slot must use real .rmBased load, matching the source's own 0.85x10RM formula")
                }
    ...
    func testMesocycleTwoDayOneMatchesRealSourceSequenceIncludingSuperset() throws {
        ...
        let slots = try XCTUnwrap(day1.orderedBlockTemplates.first).orderedPrescriptionTemplates
        XCTAssertEqual(slots.compactMap { $0.exerciseSlot?.name }, [
            "Incline Push", "Chest Isolation", "Horizontal Push", "Horizontal Push",
            "Rear or Side Delts", "Horizontal Pull",
        ], "6 real slots including the real superset partner row (Horizontal Push at index 2)")
    }

    /// Real, cell-confirmed: a THIRD distinct superset distribution
    /// pattern (after 4-Day's uniform 1-per-day and 5-Day's uneven 0/1/2
    /// spread) — this workbook has real supersets on 4 of 6 days (Days
    /// 1, 3, 5, 6), NONE on Days 2 and 4.
    func testMesocycleTwoSupersetsAppearOnFourOfSixDaysNoneOnDaysTwoAndFour() throws {
        let definition = try generate(phaseType: .metaboliteFocus)
        ...
        XCTAssertEqual(supersetPartnerCount(dayIndex: 0), 1, "Day1: one real superset partner (Horizontal Push)")
        XCTAssertEqual(supersetPartnerCount(dayIndex: 1), 0, "Day2: no real supersets")
        XCTAssertEqual(supersetPartnerCount(dayIndex: 2), 1, "Day3: one real superset partner (Horizontal Push)")
        XCTAssertEqual(supersetPartnerCount(dayIndex: 3), 0, "Day4: no real supersets")
        XCTAssertEqual(supersetPartnerCount(dayIndex: 4), 1, "Day5: one real superset partner (Biceps)")
        XCTAssertEqual(supersetPartnerCount(dayIndex: 5), 1, "Day6: one real superset partner (Rear or Side Delts)")
    }
    ...
    func testMesocycleTwoSupersetPartnersUseReducedFactorAndOmitDeload() throws {
        ...
        let partner = day1Slots[2] // Horizontal Push, the real superset partner
    ...
    func testMesocycleTwoAllSixDaysHaveRealDistinctSlotCounts() throws {
        let definition = try generate(phaseType: .metaboliteFocus)
        ...
        XCTAssertEqual(daySlotCounts, [6, 5, 6, 5, 6, 6], "34 real slots/week (6+5+6+5+6+6), matching the real recovered structure")
    }
    ...
    func testMesocycleThreeDayOneMatchesRealSourceSequence() throws {
        ...
        let slots = try XCTUnwrap(day1.orderedBlockTemplates.first).orderedPrescriptionTemplates
        XCTAssertEqual(slots.compactMap { $0.exerciseSlot?.name }, [
            "Incline Push", "Horizontal Push", "Rear or Side Delts", "Horizontal Pull",
        ])
    }

    func testMesocycleThreeAllSixDaysHaveRealReducedSlotCounts() throws {
        let definition = try generate(phaseType: .resensitization)
        let daySlotCounts = definition.orderedTemplateSessions.map { $0.orderedBlockTemplates.first?.orderedPrescriptionTemplates.count ?? 0 }
        XCTAssertEqual(daySlotCounts, [4, 4, 4, 4, 4, 5], "25 real slots/week (4+4+4+4+4+5) — Day 6 alone keeps its extra slot, the source's own genuinely reduced Mesocycle 3 volume")
    }
    ...
    /// A real 6-Day ProgramInstance must be able to progress M1 -> M2 ->
    ...
    func testMesocycleTransitionM1ToM2ToM3EachProducesItsOwnRealStructure() throws {
        ...
        XCTAssertEqual(m1Slots, [5, 5, 5, 5, 5, 5])
        XCTAssertEqual(m2Slots, [6, 5, 6, 5, 6, 6])
        XCTAssertEqual(m3Slots, [4, 4, 4, 4, 4, 5])
    ...
    /// Days 1-3, half for Days 4-6 — exactly what `DeloadStrategy`'s
    /// existing, UNMODIFIED `ceil(dayCount/2)` boundary formula already
    /// produces for a 6-day program (`ceil(6/2)=3`). Zero code change to
    ...
    func testMesocycleThreeDeloadBoundaryMatchesRealWorkbookDaySplit() throws {
        ...
        for (dayIndex, day) in definition.orderedTemplateSessions.enumerated() {
            ...
            let (deload, _) = strategy.resolveDeloadWeight(
                rules: rules, dayPositionInWeek: dayIndex, dayCount: 6,
            ...
            if dayIndex < 3 {
                ...
                XCTAssertEqual(deloadValue, 50, accuracy: 0.01, "Day \(dayIndex + 1) must be half deload weight (real workbook: Days 4-6 half)")
            }
        }
    }
    ...
    /// real 6-Day Full Body instance, never collapsing to just one
    ...
    func testCalibrationDiscoversMultipleDistinctExercisesNotJustOneExercise() throws {
        ...
        XCTAssertGreaterThan(distinctExercises.count, 1, "a real 6-Day program must require calibration for more than one exercise, unlike the legacy placeholder's collapsed single requirement")
    }
}
