...
final class PrescriptionTemplate {
    ...
    /// Stage 10R.1 Slice 1B addition — see `AutoregulatedSetCount
    /// .treatMissingRatingAsNoChange`'s doc comment. `false` for every
    /// pre-existing row (Family A/B/C, unchanged behavior).
    var setCountRuleTreatMissingRatingAsNoChange: Bool = false

    /// Parallel arrays, index-aligned — `RepGoal` is a plain struct, so it
    /// is not implicated in either failure mode above, but is kept flat
    /// anyway for consistency and because arrays of primitives are the
    /// single most-proven-safe shape in this codebase.
    /// Stage 10R.1D: `repGoalReps: [Int]` + `repGoalToFailure: [Bool]`
    /// (a fabricated-rep-count-vs-failure-flag pair) replaced by an
    /// explicit discriminator + one shared value slot — `repGoalIsFixedReps[i]
    /// == true` means `repGoalPrescriptionValue[i]` is a real fixed rep
    /// count (`RepPrescriptionKind.fixedReps`); `false` means it's an RIR
    /// target (`.rir`). No stored user data exists yet to migrate (see
    /// this stage's persistence audit) — every not-yet-materialized
    /// `PrescriptionTemplate` row is regenerated by its generator, never
    /// migrated.
        ...
        get {
            ...
            case .autoregulated:
                ...
                return .autoregulated(AutoregulatedSetCount(
                    ...
                    treatMissingRatingAsNoChange: setCountRuleTreatMissingRatingAsNoChange
                ))
            }
        }
        set {
            ...
            setCountRuleTreatMissingRatingAsNoChange = false
            switch newValue {
            case .fixed(let sets):
            ...
            case .autoregulated(let config):
                ...
                setCountRuleTreatMissingRatingAsNoChange = config.treatMissingRatingAsNoChange
            case nil:
    ...
    var repGoalSchedule: [RepGoal] {
        ...
        set {
            repGoalIsFixedReps = newValue.map {
                if case .fixedReps = $0.prescription { return true } else { return false }
            }
            repGoalPrescriptionValue = newValue.map {
